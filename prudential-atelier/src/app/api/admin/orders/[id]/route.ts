...
import { isFabricUnavailableChoice } from "@/lib/fabric-unavailable";
import { notifyFabricUnavailable } from "@/lib/notifications";
...
const fabricUnavailableSchema = z.object({
  fabricUnavailable: z.object({
    choice: z.string(),
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const fabricTry = fabricUnavailableSchema.safeParse(body);
  if (fabricTry.success) {
    const order = await prisma.order.findUnique({ where: { id } });
    if (!order) return NextResponse.json({ error: "Not found" }, { status: 404 });
    const choice = fabricTry.data.fabricUnavailable.choice;
    if (!isFabricUnavailableChoice(choice)) {
      return NextResponse.json({ error: "Invalid fabric choice" }, { status: 400 });
    }
    const actorId = gate.session.user.id;
    const actorName =
      [gate.session.user.name, gate.session.user.email].filter(Boolean).join(" · ") || actorId;
    const note = fabricTry.data.fabricUnavailable.note?.trim() || null;
    const stamp = new Date().toISOString().slice(0, 10);
    const line = `\n[${stamp}] Fabric unavailable recorded by ${actorName}: ${choice}${note ? `. ${note}` : ""}`;
    const adminNotes = [order.adminNotes?.trim() ?? "", line].filter(Boolean).join("\n");
    const updated = await prisma.order.update({
      where: { id },
      data: {
        fabricUnavailableAt: order.fabricUnavailableAt ?? new Date(),
        fabricUnavailableChoice: choice,
        fabricUnavailableById: actorId,
        fabricUnavailableByName: actorName,
        fabricUnavailableNote: note,
        adminNotes,
      },
    });
    notifyFabricUnavailable({ id: updated.id, orderNumber: updated.orderNumber });
    return NextResponse.json(updated);
  }
